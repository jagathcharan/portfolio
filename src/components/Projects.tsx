...
function Projects() {
  ...
  return (
    <section id="projects" className="py-16 sm:py-20 lg:py-24 relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
            ...
            {projects.map((project, index) => (
              ...
              >
                <div className="flex items-start justify-between mb-3 sm:mb-4">
                  <h3 className="text-base sm:text-lg lg:text-xl font-bold text-white group-hover:gradient-text transition-all duration-300 flex-1 pr-2">
                    {project.title}
                  </h3>
                  <ExternalLink size={18} className="text-ai-primary opacity-0 group-hover:opacity-100 transition-all duration-300 flex-shrink-0 mt-1 group-hover:translate-x-1 group-hover:-translate-y-1" />
                </div>
                ...
                <div className="mb-3 sm:mb-4">
                  <p className="text-xs text-ai-secondary font-semibold mb-1">Role: {project.role}</p>
                  <p className="text-xs text-slate-500 italic">{project.impact}</p>
                </div>

                <div className="flex flex-wrap gap-1.5 pt-3 sm:pt-4 border-t border-slate-800">
                  {project.tech.map((tech, techIndex) => (
                    <span
                      key={techIndex}
                      className="px-2 py-0.5 bg-ai-primary/10 text-ai-primary rounded-lg text-xs font-medium border border-ai-primary/20 hover:bg-ai-primary/20 hover:border-ai-primary/40 transition-colors duration-200"
                    >
                      {tech}
                    </span>
                  ))}
